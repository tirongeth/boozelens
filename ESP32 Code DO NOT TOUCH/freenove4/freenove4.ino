...
void sendBACToFirebase(float bacValue) {
  ...
  HTTPClient http;
  String url = "https://" + String(firebaseHost) + "/readings/" + deviceId + "/bac.json?auth=" + String(firebaseAuth);
  ...
  if(httpCode == 200) {
    Serial.println("BAC uploaded to Firebase");
  } else {
...
void sendPhotoMetadataToFirebase(const char* filename, size_t fileSize) {
  ...
  HTTPClient http;
  String url = "https://" + String(firebaseHost) + "/readings/" + deviceId + "/photos.json?auth=" + String(firebaseAuth);
  ...
  if(httpCode == 200 || httpCode == 201) {
    Serial.println("Photo metadata uploaded to Firebase");
  } else {
    Serial.printf("Photo metadata upload failed: %d\n", httpCode);
  }
